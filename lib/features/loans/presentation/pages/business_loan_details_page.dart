...
class BusinessLoanDetailsPage extends StatelessWidget {
  final LoanModel loan;
  const BusinessLoanDetailsPage({super.key, required this.loan});

  static const _primary = Color(0xFF0D47A1);
  static const _bg      = Color(0xFFE3F2FD);
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            ...
            title: Text(
              ...
              style: TextStyle(color: _primary, fontSize: 16.sp, fontWeight: FontWeight.w700),
            ),
            centerTitle: true,
          ),
    ...
    return Container(
      ...
      child: Row(children: [
        Container(
          ...
          child: Text(initial, style: TextStyle(color: _primary, fontWeight: FontWeight.w800, fontSize: 18.sp)),
        ),
        SizedBox(width: 12.w),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(name, style: TextStyle(fontSize: 16.sp, fontWeight: FontWeight.w700, color: Colors.black87),
              maxLines: 1, overflow: TextOverflow.ellipsis),
          SizedBox(height: 2.h),
          Text(role, style: TextStyle(fontSize: 11.sp, color: _accent, fontWeight: FontWeight.w600)),
          if (phone.isNotEmpty) ...[
            SizedBox(height: 2.h),
            Row(children: [
              Icon(Icons.phone_outlined, size: 12.sp, color: Colors.grey),
              SizedBox(width: 4.w),
              Flexible(child: Text(phone, style: TextStyle(fontSize: 12.sp, color: Colors.grey.shade600),
        ...
        if (phone.isNotEmpty)
          Row(mainAxisSize: MainAxisSize.min, children: [
            _iconAction(icon: Icons.phone, color: Colors.blue.shade600, onTap: () {
              Clipboard.setData(ClipboardData(text: phone));
            }),
            SizedBox(width: 12.w),
            _iconAction(icon: Icons.chat_bubble_outline, color: Colors.green.shade600, onTap: () async {
  ...
  Widget _iconAction({required IconData icon, required Color color, required VoidCallback onTap}) {
    return GestureDetector(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        SizedBox(height: 4.h),
        Text(icon == Icons.phone ? 'Call' : 'WhatsApp',
    ...
    return Container(
      ...
      child: Column(children: [
        Padding(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Container(
              ...
              decoration: BoxDecoration(color: _bg, borderRadius: BorderRadius.circular(20.r)),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.business_center_outlined, size: 14.sp, color: _primary),
                SizedBox(width: 6.w),
                Text('Business Credit', style: TextStyle(color: _primary, fontSize: 12.sp, fontWeight: FontWeight.w700)),
              ]),
            ),
        ...
        Padding(
          ...
          child: Column(children: [
            Row(children: [
              Expanded(child: _stat('Principal Amount', '₹${_fmt(loan.amount)}')),
  ...
  Widget _stat(String label, String value, {Color? valueColor}) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      SizedBox(height: 4.h),
      Text(value, style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.w700, color: valueColor ?? Colors.black87)),
    ]);
  }
  ...
  Widget _statusChip(String status) {
    ...
    return Container(
      ...
      child: Text(_statusLabel(status), style: TextStyle(color: color, fontSize: 11.sp, fontWeight: FontWeight.w700)),
    );
  }
  ...
  Widget _emiOverviewCard(LoanModel loan) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Icon(Icons.account_balance_wallet_outlined, color: Colors.white70, size: 16.sp),
          SizedBox(width: 6.w),
          Text('EMI Overview', style: TextStyle(color: Colors.white70, fontSize: 12.sp, fontWeight: FontWeight.w600)),
        ]),
        SizedBox(height: 16.h),
        Row(children: [
          Expanded(child: _gradientStat('Monthly EMI', emi > 0 ? '₹${_fmt(emi)}' : '-')),
          Expanded(child: _gradientStat('Total Payable', totalPayable > 0 ? '₹${_fmt(totalPayable)}' : '-')),
        ]),
        SizedBox(height: 12.h),
        Row(children: [
          Expanded(child: _gradientStat('Amount Paid', '₹${_fmt(amountPaid)}')),
          Expanded(child: _gradientStat('Amount Pending', '₹${_fmt(amountPending)}')),
        ]),
        SizedBox(height: 12.h),
        Row(children: [
          Expanded(child: _gradientStat('Paid Months', '$paidMonths / $duration')),
          Expanded(child: _gradientStat('Remaining', '${duration - paidMonths} months')),
        ]),
      ]),
    );
  }
  ...
  Widget _paymentChecklist(LoanModel loan) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Monthly Payments', style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.w700, color: Colors.black87)),
        SizedBox(height: 4.h),
        Text('$paidMonths of $duration months paid', style: TextStyle(fontSize: 12.sp, color: Colors.grey.shade500)),
        SizedBox(height: 12.h),
        ClipRRect(
          borderRadius: BorderRadius.circular(8.r),
          child: LinearProgressIndicator(
              ...
              return Padding(
                ...
                child: Row(children: [
                  Container(
                    ...
                    alignment: Alignment.center,
                    child: Icon(isPaid ? Icons.check : Icons.calendar_today_outlined,
                        size: 14.sp, color: isPaid ? _primary : Colors.grey),
                  ),
                  SizedBox(width: 12.w),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('Month ${index + 1} — ${_month(dueDate.month)} ${dueDate.year}',
                        style: TextStyle(fontSize: 13.sp, fontWeight: FontWeight.w600, color: Colors.black87)),
                    Text('Due: ${dueDate.day} ${_month(dueDate.month)} ${dueDate.year}',
  ...
  Widget _proofDocumentSection(BuildContext context, LoanModel loan) {
    if (loan.documentUrl == null || loan.documentUrl!.trim().isEmpty) return const SizedBox.shrink();
          ...
          child: Container(
            ...
            child: Row(children: [
              Container(
                ...
                decoration: BoxDecoration(color: _bg, borderRadius: BorderRadius.circular(8.r)),
                clipBehavior: Clip.antiAlias,
                child: isPdf
                    ? const Icon(Icons.picture_as_pdf, color: Color(0xFFDC2626), size: 22)
                    : Image.network(url, fit: BoxFit.cover, cacheWidth: 200,
                        loadingBuilder: (_, child, p) => p == null ? child : const Center(child: CircularProgressIndicator(strokeWidth: 2)),
                        errorBuilder: (_, __, ___) => const Icon(Icons.image, color: _primary, size: 22)),
              ),
              SizedBox(width: 12.w),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(isPdf ? 'Signed_Agreement_Business_Credit.pdf' : 'Proof_Document_Business_Credit.jpg',
                    style: TextStyle(fontSize: 12.sp, fontWeight: FontWeight.bold, color: Colors.black87),
                    maxLines: 1, overflow: TextOverflow.ellipsis),
                SizedBox(height: 2.h),
                Text('Tap to view proof document', style: TextStyle(fontSize: 10.sp, color: Colors.grey.shade600)),
              ])),
      ...
      builder: (context) => Dialog(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Container(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Expanded(child: Text(isPdf ? 'Agreement & Signature' : 'Proof Document',
                  style: TextStyle(fontSize: 16.sp, fontWeight: FontWeight.bold, color: Colors.black87))),
              IconButton(icon: const Icon(Icons.close, color: Colors.black54),
                  onPressed: () => Navigator.of(context).pop(), padding: EdgeInsets.zero, constraints: const BoxConstraints()),
            ]),
          ),
          Flexible(
            child: Container(
      ...
      child: ClipRRect(
        ...
        child: InteractiveViewer(
          ...
          child: Image.network(url, fit: BoxFit.contain, cacheWidth: 1080,
              loadingBuilder: (_, child, p) => p == null ? child : const Center(child: CircularProgressIndicator(color: _primary)),
  ...
  Widget _pdfPreview() {
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('PDF Document', style: TextStyle(fontSize: 16.sp, fontWeight: FontWeight.bold, color: Colors.black87)),
        SizedBox(height: 8.h),
        Text('Tap "Open External" to view in your browser or PDF reader.',
            textAlign: TextAlign.center, style: TextStyle(fontSize: 12.sp, color: Colors.grey.shade600, height: 1.4)),
      ]),
    );
  }
  ...
  Color _statusColor(String s) {
    switch (s.toLowerCase()) {
      ...
      case 'closed': case 'completed': return Colors.grey.shade600;
      case 'pending_otp': case 'pending_approval': return const Color(0xFFD97706);
