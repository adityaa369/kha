...
class _ChitGroupAdminPageState extends State<ChitGroupAdminPage> {
          ...
          return TabBarView(
            children: [
               RefreshIndicator(
                 ...
                 onRefresh: () => context.read<ChitFundCubit>().loadAdminDashboard(widget.chitId),
                 child: ListView(
  ...
  void _showFinalizeAuctionSheet(Map<String, dynamic> auction, List members, Map<String, dynamic> chitDetails) {
     ...
     final Future<List<Map<String, dynamic>>> bidsFuture = context.read<ChitFundCubit>().getAuctionBids(chitDetails['id'], auction['monthNumber']);
            ...
            return FutureBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (ctx, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return Container(height: 200.h, alignment: Alignment.center, child: const CircularProgressIndicator());
                }
                          ...
                          DropdownButtonFormField<String>(
                             ...
                             items: bids.map((b) {
                                ...
                                return DropdownMenuItem<String>(
                                   value: user['id'],
                                   child: Text('${user['firstName']} ${user['lastName']} - Bid: ₹${b['bidDiscount']}'),
                                );
                             }).toList(),
                             onChanged: (val) {
                                selectedUserId = val;
                                final b = bids.firstWhere((x) => x['user']['id'] == val);
                                _bidController.text = b['bidDiscount'].toString();
                             },
  ...
  Widget _buildAuctionTimeline(List auctions, List members, Map<String, dynamic> chitDetails) {
    ...
    bool isOwner = chitDetails['isOwner'] == true;
    int activeAuctionMonth = int.tryParse(chitDetails['activeAuctionMonth']?.toString() ?? '0') ?? 0;
    ...
    final bool hasWonAuction = myMemberObj != null && myMemberObj['hasWonAuction'] == true;
    ...
    return Padding(
      ...
      child: Column(
        children: auctions.map((auction) {
          ...
          final int auctionMonth = int.tryParse(auction['monthNumber']?.toString() ?? '0') ?? 0;
          final bool isAuctionOpened = activeAuctionMonth == auctionMonth && activeAuctionMonth != 0;
            ...
            child: Column(
              children: [
                            ...
                            if (isCompleted && auction['winner'] != null)
                              ...
                              Text(
                               ...
                               Text(
                                 ...
                                 style: TextStyle(color: KhaataTheme.primaryBlue, fontSize: 13.sp, fontWeight: FontWeight.bold),
                               )
                            else if (isActive)
                              Text(
                ...
                if (isActive && !isOwner && isAuctionOpened) ...[
                   ...
                   const Divider(),
                   SizedBox(height: 8.h),
                   _UserBidForm(chitDetails: chitDetails, auction: auction, hasWonAuction: hasWonAuction),
                ]
              ],
            ),
          );
...
class _UserBidForm extends StatefulWidget {
  ...
  const _UserBidForm({required this.chitDetails, required this.auction, required this.hasWonAuction});
        ...
        children: [
          ...
          if (widget.hasWonAuction) 
             Container(
               width: double.infinity,
               padding: EdgeInsets.all(12.w),
               decoration: BoxDecoration(color: KhaataTheme.dangerRed.withOpacity(0.1), borderRadius: BorderRadius.circular(8.r)),
               child: Row(
                 crossAxisAlignment: CrossAxisAlignment.start,
                 children: [
                   Icon(Icons.block, color: KhaataTheme.dangerRed, size: 20.sp),
                   SizedBox(width: 8.w),
                   Expanded(child: Text('You have already won a previous auction. You are not eligible to bid.', style: TextStyle(color: KhaataTheme.dangerRed, fontSize: 13.sp))),
                 ],
               )
             )
          else ...[
