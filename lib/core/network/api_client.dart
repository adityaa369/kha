            ...
            switch (e.type) {
              ...
              case DioExceptionType.badResponse:
                ...
                if (statusCode == 503) {
                  ...
                  myException = e.copyWith(
                    error: ServerFailure(
                      msg ?? 'Financial operations are temporarily suspended.',
                      code,
                    ),
                  );
                  break;
                ...
                if (code == 'RATE_LIMITED' || statusCode == 429) {
                  ...
                  myException = e.copyWith(
                    error: IntentConsumedFailure(
                      msg ?? 'Action already completed',
                      code,
                    ),
                  );
                } else if (code == 'OVERPAYMENT_REJECTED' ||
                    code == 'LOAN_FROZEN' ||
                    code == 'TERMINAL_STATE' ||
                    code == 'BUSINESS_ERROR') {
                  myException = e.copyWith(
                    error: BusinessLogicFailure(
                  ...
                  myException = e.copyWith(
                    error: AuthFailure(msg ?? 'Session expired', code),
                  );
                } else if (code == 'FORBIDDEN' || statusCode == 403) {
                  myException = e.copyWith(
                    error: AuthFailure(msg ?? 'Access denied', code),
                  );
                } else if (statusCode >= 500) {
                  myException = e.copyWith(
                    error: ServerFailure(msg ?? 'Internal Server Error', code),
                  );
                } else {
                  myException = e.copyWith(
                    error: ValidationFailure(msg ?? 'Request failed', code),
                  );
                }
                break;
              default:
