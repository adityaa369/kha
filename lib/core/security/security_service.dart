...
class SecurityService {
  ...
  static Future<void> preventScreenshots() async {
    ...
    try {
      ...
      // Noncritical — don't crash the app if this fails
    }
  }
  ...
  static Future<bool> _isDeviceCompromised() async {
    ...
    // For productiongrade: add flutter_jailbreak_detection to pubspec.yaml
    try {
      // Check for common rootjailbreak indicators
      ...
      for (final file in suspiciousFiles) {
        try {
          // This will throw if file doesnt exist (which is the normal/safe case)
          await const MethodChannel(
            'khatha/security',
          ).invokeMethod<bool>('checkFile', {'path': file});
          return true; // File exists  potentially rooted
        } catch (_) {
          // File doesnt exist or channel not implemented — continue
        }
      }
      return false;
    } catch (_) {
  ...
  static void initialize() {
    if (kReleaseMode) {
      // Disable Flutters error detail reporting in release mode
