...
class PaymentFlowCubit extends Cubit<PaymentFlowState> {
  ...
  PaymentFlowCubit({required LoanRepository repository, required String loanId})
  ...
  Future<void> submitPayment(int amountPaise) async {
    ...
    if (state is PaymentIdle &&
        (state as PaymentIdle).failedAttempt != null &&
        (state as PaymentIdle).failedAttempt!.amountPaise == amountPaise) {
      ...
      if (success) {
        ...
        emit(
          const PaymentRejected(
            ServerFailure('Payment failed without specific error'),
          ),
        );
      }
    } on TimeoutException {
    ...
    if (e.response?.data is Map && e.response?.data['code'] != null) {
      ...
      switch (code) {
        ...
        case 'OVERPAYMENT_REJECTED':
          return const BusinessLogicFailure('OVERPAYMENT_REJECTED');
        case 'LOAN_FROZEN':
