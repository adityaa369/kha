...
class CloseLoanFlowCubit extends Cubit<CloseLoanFlowState> {
  ...
  CloseLoanFlowCubit({
    ...
    String? initialIntentId,
       ...
       _loanId = loanId,
       super(initialIntentId != null ? CloseLoanAwaitingConsent(initialIntentId) : CloseLoanIdle());
...
// 4F-4G: Fetch Intent for Deep Linking
  Future<void> loadIntent(String intentId) async {
    ...
    try {
      ...
      if (intent.action != 'CLOSE_LOAN') {
        emit(const CloseLoanRejectedState(BusinessLogicFailure('INVALID_INTENT_TYPE')));
        return;
      }
      ...
      if (intent.status == 'CONSUMED' || intent.status == 'COMMITTED') {
        ...
        emit(const CloseLoanRejectedState(BusinessLogicFailure('INTENT_EXPIRED')));
      } else if (intent.status == 'REJECTED') {
        emit(const CloseLoanRejectedState(BusinessLogicFailure('INTENT_REJECTED')));
      } else if (intent.status == 'PENDING') {
        emit(CloseLoanAwaitingConsent(intentId));
      } else {
        emit(const CloseLoanRejectedState(ServerFailure('Unknown intent status')));
      }
  ...
  Future<void> approveIntent() async {
    ...
    try {
      ...
      final success = await _repository.commitClose(loanId: _loanId, intentId: intentId);
  ...
  Future<void> rejectIntent() async {
    ...
    try {
      ...
      emit(CloseLoanSuccess()); // Successfully rejected (doesn't change loan state)
    } catch (e) {
      emit(CloseLoanSuccess());
    }
  }
  ...
  Failure _mapDioErrorToFailure(DioException e) {
    if (e.response?.data is Map && e.response?.data['code'] != null) {
      ...
      switch (code) {
        ...
        case 'AUTH_REQUIRED': return const BusinessLogicFailure('AUTH_REQUIRED');
      }
    }
