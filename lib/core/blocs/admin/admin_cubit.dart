...
class AdminCubit extends Cubit<AdminState> {
  ...
  AdminCubit({ApiClient? api}) : _api = api ?? ApiClient(), super(AdminInitial());
  ...
  Future<void> loadChitFunds({int page = 1}) async {
    emit(AdminLoading());
    try {
      final response = await _api.get('/admin/chit-funds', queryParameters: {'page': page});
      final data = response.data;
      emit(AdminChitsLoaded(
  ...
  Future<void> suspendUser(String userId, bool suspend) async {
    try {
      await _api.put('/admin/users/$userId/suspend', data: {'suspend': suspend});
