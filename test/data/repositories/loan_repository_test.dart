...
void main() {
  ...
  group('4F-3 API Contract Tests', () {
    test(
      '1 & 2: Valid idToken is sent, legacy otp is removed for recordPayment',
      () async {
        when(
          () => mockUser.getIdToken(any()),
        ).thenAnswer((_) async => 'mock_firebase_token');
    ...
    test(
      '3 & 4: Expired Firebase ID token refreshes correctly and retries',
      () async {
        when(
        ...
        final capturedFetch = verify(
          () => mockDio.fetch(captureAny()),
        ).captured;
