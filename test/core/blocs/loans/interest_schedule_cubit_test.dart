...
void main() {
  ...
  group('4F-4E Interest UI & Accrual Presentation', () {
    test('4 & 5 & 6. Accrued, Paid, Outstanding interest originate from backend ledger', () async {
      ...
      const authoritativeSchedule = InterestScheduleModel(
        ...
        schedule: [
          InterestPeriodViewModel(month: 'August 2026', accruedPaise: 99, paidPaise: 50),
        ],
      );

      when(() => mockRepo.getInterestSchedule('loan123'))
    ...
    test('13. Network failure preserves last known values', () async {
      // First load succeeds
      const authoritativeSchedule = InterestScheduleModel(
        totalAccruedPaise: 99, totalPaidPaise: 50, outstandingInterestPaise: 49,
        originalPrincipalPaise: 100000, interestRateBps: 1200, interestMethod: 'SIMPLE',
        schedule: [],
      );
      when(() => mockRepo.getInterestSchedule('loan123')).thenAnswer((_) async => authoritativeSchedule);
      await cubit.fetchSchedule('loan123');
      
      // Second load fails
      when(() => mockRepo.getInterestSchedule('loan123')).thenThrow(const NetworkFailure());
      await cubit.fetchSchedule('loan123');
