...
void main() {
  ...
  group('4F-4B Payment Flow Validation', () {
    test('1. successful payment', () async {
      when(() => mockRepo.recordPayment('loan1', amountRupees: 5000.0, idempotencyKey: any(named: 'idempotencyKey')))
    ...
    test('2. failed payment leaves state unchanged (emits rejected)', () async {
      ...
      expect((cubit.state as PaymentRejected).failure, isA<BusinessLogicFailure>());
    });
    ...
    test('3. duplicate tap', () async {
      ...
      verify(() => mockRepo.recordPayment('loan1', amountRupees: 5000.0, idempotencyKey: any(named: 'idempotencyKey'))).called(1);
    });
    ...
    test('5. UNKNOWN -> backend says committed', () async {
      ...
      when(() => mockRepo.checkTransactionStatus(unknownState.attempt.idempotencyKey))
    ...
    test('6. UNKNOWN -> backend says not committed & 7. retry uses SAME idempotency key', () async {
      when(() => mockRepo.recordPayment('loan1', amountRupees: 5000.0, idempotencyKey: any(named: 'idempotencyKey')))
          .thenThrow(TimeoutException('timeout'));
      ...
      when(() => mockRepo.checkTransactionStatus(originalKey))
      ...
      when(() => mockRepo.recordPayment('loan1', amountRupees: 5000.0, idempotencyKey: originalKey))
          .thenAnswer((_) async => true);
      ...
      verify(() => mockRepo.recordPayment('loan1', amountRupees: 5000.0, idempotencyKey: originalKey)).called(2);
    });
    ...
    test('8. Firebase token refresh preserves same request/key', () async {
      ...
      when(() => mockRepo.recordPayment('loan1', amountRupees: 5000.0, idempotencyKey: key))
          .thenAnswer((_) async => true);
    ...
    test('9, 10, 11, 12, 13, 14. structured errors handled correctly', () async {
      final dioRateLimit = DioException(
        requestOptions: RequestOptions(path: ''),
        response: Response(requestOptions: RequestOptions(path: ''), data: {'code': 'RATE_LIMITED'}),
      );
      when(() => mockRepo.recordPayment('loan1', amountRupees: 5000.0, idempotencyKey: any(named: 'idempotencyKey')))
      ...
      await cubit.submitPayment(5000.0);
      expect(cubit.state, isA<PaymentRejected>());
      expect((cubit.state as PaymentRejected).failure, isA<RateLimitedFailure>());
    });
  });
}
