...
void main() {
  ...
  group('4F-4C Add Credit Validation', () {
    test('1 & 2. Create Add Credit intent -> Lender sees "Awaiting consent"', () async {
      final cubit = AddCreditFlowCubit(repository: mockRepo, loanId: 'loan1');
      
      when(() => mockRepo.createAddCreditIntent(loanId: 'loan1', amountRupees: 5000.0))
    ...
    test('3 & 4. Borrower loads intent -> Correct borrower can authorize', () async {
      ...
      when(() => mockRepo.commitAddCredit(loanId: 'loan1', intentId: 'intent123', amountRupees: 5000.0))
    ...
    test('6. Intent amount cannot be tampered with', () async {
      ...
      await cubit.approveIntent();
      verify(() => mockRepo.commitAddCredit(loanId: 'loan1', intentId: 'intent123', amountRupees: 5000.0)).called(1);
    });

    test('7. Expired intent cannot authorize', () async {
      final cubit = AddCreditFlowCubit(
        repository: mockRepo, loanId: 'loan1', initialIntentId: 'intent123', initialAmountRupees: 5000.0,
      );
      ...
      expect((cubit.state as AddCreditRejectedState).failure, isA<BusinessLogicFailure>());
      expect(((cubit.state as AddCreditRejectedState).failure as BusinessLogicFailure).message, 'INTENT_EXPIRED');
    });
    ...
    test('8. Consumed intent cannot authorize', () async {
      ...
      await cubit.approveIntent();
      expect(cubit.state, isA<AddCreditRejectedState>());
      expect((cubit.state as AddCreditRejectedState).failure, isA<IntentConsumedFailure>());
    });
    ...
    test('14. Rejected intent causes zero financial delta', () async {
      ...
      when(() => mockRepo.rejectIntent('intent123'))
          .thenAnswer((_) async {});
      ...
      verifyNever(() => mockRepo.commitAddCredit(loanId: any(named: 'loanId'), intentId: any(named: 'intentId'), amountRupees: any(named: 'amountRupees')));
    });
    ...
    test('15 & 16. Frozen or Terminal loan blocks intent/commit', () async {
      ...
      await cubit.createIntent(5000.0);
      expect(cubit.state, isA<AddCreditRejectedState>());
      expect(((cubit.state as AddCreditRejectedState).failure as BusinessLogicFailure).message, 'LOAN_FROZEN');
    });
  });
}
