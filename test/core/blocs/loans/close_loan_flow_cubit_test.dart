...
void main() {
  ...
  group('4F-4D Close Loan Validation', () {
    test('1 & 6. Close loan -> Intent created -> Awaiting consent', () async {
      ...
      when(
        () => mockRepo.createCloseIntent(loanId: 'loan1'),
    ...
    test(
      '3 & 6. Borrower loads intent -> Correct borrower can authorize',
      () async {
        ...
        when(
          () => mockRepo.commitClose(loanId: 'loan1', intentId: 'intent123'),
    ...
    test('8. Expired intent cannot authorize', () async {
      ...
      expect(
        (cubit.state as CloseLoanRejectedState).failure,
    ...
    test(
      '13 & 14. Network timeout reconciles existing intent -> COMMITTED',
      () async {
        final cubit = CloseLoanFlowCubit(
          repository: mockRepo,
          loanId: 'loan1',
    ...
    test(
      '15. Network timeout reconciles existing intent -> NOT COMMITTED (PENDING)',
      () async {
        final cubit = CloseLoanFlowCubit(
          repository: mockRepo,
          loanId: 'loan1',
    ...
    test(
      '20. No local accounting mutation (logic relies only on cubit state, no math)',
      () async {
        final cubit = CloseLoanFlowCubit(
          repository: mockRepo,
          loanId: 'loan1',
