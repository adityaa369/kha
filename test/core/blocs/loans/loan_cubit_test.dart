...
void main() {
  ...
  group('4F-4A LoanCubit Synchronization Gates', () {
    test('successful payment refreshes authoritative balance', () async {
      when(() => mockRepo.fetchLoans()).thenAnswer((_) async => {
        'myLoans': [initialLoan], 'givenLoans': []
      });
      
      await cubit.fetchLoans();
      expect((cubit.state as LoansLoaded).myLoans.first.totalOutstandingAmount, 10000.0);

      // Setup payment success and subsequent fetch
      when(() => mockRepo.recordPayment('loan1', amountRupees: 3000.0)).thenAnswer((_) async => true);
      when(() => mockRepo.fetchLoans()).thenAnswer((_) async => {
        'myLoans': [loanAfterPayment], 'givenLoans': []
      });
      ...
      expect((cubit.state as LoansLoaded).myLoans.first.totalOutstandingAmount, 7000.0);
      expect((cubit.state as LoansLoaded).myLoans.first.paidAmountPaise, 300000);
    ...
    test('failed payment doesn\'t mutate balance', () async {
      ...
      when(() => mockRepo.recordPayment('loan1', amountRupees: 3000.0)).thenThrow(const BusinessLogicFailure('OVERPAYMENT_REJECTED'));
    ...
    test('timeout doesn\'t optimistically mutate balance', () async {
      ...
      when(() => mockRepo.recordPayment('loan1', amountRupees: 3000.0)).thenThrow(TimeoutException('Timeout'));
    ...
    test('successful Add Credit refreshes state', () async {
      ...
      when(() => mockRepo.addCredit('loan1', amountRupees: 1000.0)).thenAnswer((_) async => true);
      
      const higherLoan = LoanModel(
        id: 'loan1', borrowerName: 'Alice', initials: 'AL',
        amountPaise: 1100000, principalOutstandingPaise: 1100000,
        status: 'active', progress: 0.0, type: 'personal'
      );
      when(() => mockRepo.fetchLoans()).thenAnswer((_) async => {
        'myLoans': [higherLoan], 'givenLoans': []
      });

      await cubit.addCredit('loan1', amountRupees: 1000.0);
      expect((cubit.state as LoansLoaded).myLoans.first.totalOutstandingAmount, 11000.0);
    });
    ...
    test('successful Close changes state only from backend response', () async {
      ...
      const closedLoan = LoanModel(
        id: 'loan1', borrowerName: 'Alice', initials: 'AL',
        amountPaise: 1000000, principalOutstandingPaise: 0,
        status: 'closed', progress: 1.0, type: 'personal'
      );
      when(() => mockRepo.fetchLoans()).thenAnswer((_) async => {
        'myLoans': [closedLoan], 'givenLoans': []
      });

      await cubit.closeLoan('loan1');
      expect((cubit.state as LoansLoaded).myLoans.first.status, 'closed');
      expect((cubit.state as LoansLoaded).myLoans.first.totalOutstandingAmount, 0.0);
    });

    test('terminal loan stays terminal in UI', () async {
      const defaultedLoan = LoanModel(
        id: 'loan2', borrowerName: 'Bob', initials: 'BO',
        amountPaise: 1000000, principalOutstandingPaise: 1000000,
        status: 'defaulted', progress: 0.0, type: 'personal'
      );
